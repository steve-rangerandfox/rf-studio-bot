...
async function resolveUserId(email, token) {
  ...
  if (!res.ok) {
    ...
    if (res.status === 403) {
      throw new Error(`Permission denied resolving user "${email}". Azure app needs User.Read.All (Application) permission with admin consent.`);
    }
...
export async function provisionTeamsChat(env, form) {
  ...
  try {
    ...
    return { service, success: true, url, id: chatId };
  } catch (err) {
