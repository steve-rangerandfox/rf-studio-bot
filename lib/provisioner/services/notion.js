...
export async function createNotionPage(env, form) {
  ...
  try {
    ...
    // Client — from the form's clientName field
    if (form.clientName) {
      setIfExists(properties, schema, "Client", "select",
        { select: { name: form.clientName } });
      // Also try as rich_text in case the column type differs
      setIfExists(properties, schema, "Client", "rich_text",
        { rich_text: [{ text: { content: form.clientName } }] });
    }
    ...
    logger.info(`Notion setting properties: ${Object.keys(properties).join(", ")}`);
