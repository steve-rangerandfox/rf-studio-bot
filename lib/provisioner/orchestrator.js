...
export async function runOrchestrator(env, ctx, onProgress) {
  ...
  // -- Phase 2: Notion --
  console.log("[orchestrator] Starting Phase 2 (Notion)...");
  try {
    if (sel.has("notion")) {
    ...
    console.error("[orchestrator] Phase 2 crashed:", err.message);
    results.notion = { service: "Notion", success: false, error: err.message };
  }

  logger.info(`Phase 2 complete: ${statusIcon(results.notion)} notion`);

  if (onProgress) {
    try { await onProgress("phase2_complete", { ...results }); } catch (_) {}
  }

  // -- Phase 3: Teams Chat --
  console.log("[orchestrator] Starting Phase 3 (Teams Chat)...");
  try {
    if (sel.has("teams")) {
    ...
    console.error("[orchestrator] Phase 3 crashed:", err.message);
    results.teams = { service: "Teams", success: false, error: err.message };
  }

  logger.info(`Phase 3 complete: ${statusIcon(results.teams)} teams`);

  if (onProgress) {
    try { await onProgress("phase3_complete", { ...results }); } catch (_) {}
  }

  // -- Phase 4: Patch Notion with links from other services --
  console.log("[orchestrator] Starting Phase 4 (Notion link patching)...");
  try {
    ...
    console.error("[orchestrator] Phase 4 crashed:", err.message);
  }

  console.log("[orchestrator] All phases complete, sending summary...");

  if (onProgress) {
    try { await onProgress("phase4_complete", { ...results }); } catch (_) {}
  }
