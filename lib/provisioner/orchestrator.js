...
export async function runOrchestrator(env, ctx, onProgress) {
  ...
  // -- Phase 1: Parallel provisioning (6 services) --
  const phase1 = await Promise.allSettled([
    ...
    maybeRun("canva", () => provisionCanva(env, form.projectName, form.clientName), "Canva"),
    ...
    maybeRun("figma", () => provisionFigma(env, form.projectName, form.clientName), "FigJam"),
  ]);

  const keys = ["dropbox", "frameio", "canva", "onedrive", "clockify", "figma"];
  const names = ["Dropbox", "FrameIo", "Canva", "OneDrive", "Clockify", "FigJam"];
  ...
  if (sel.has("notion") && results.notion?.success && results.notion.id) {
    const links = {
      ...
      canvaUrl: results.canva?.success ? results.canva.url : undefined,
      ...
      figjamUrl: results.figma?.success ? results.figma.url : undefined,
    };
