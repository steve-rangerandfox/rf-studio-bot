...
async function runProvisioningAsync(env, convId, serviceUrl, form) {
  ...
  const icon = (r) => !r ? "" : r.error === "skipped" ? "\u23ed\ufe0f" : r.success ? "\u2705" : "\u274c";

  const onProgress = async (phase, results) => {
    try {
      if (phase === "phase1_complete") {
        const phase1Keys = ["dropbox", "frameio", "onedrive", "clockify"];
        const line = phase1Keys
          .filter((k) => results[k])
          .map((k) => `${icon(results[k])} ${SERVICE_EMOJI[k] || k}`)
          .join(" ");
        await sendText(env, serviceUrl, convId, `**Phase 1 complete:** ${line}`);
      } else if (phase === "phase2_complete") {
        await sendText(env, serviceUrl, convId, `**Phase 2:** ${icon(results.notion)} Notion`);
      } else if (phase === "phase3_complete") {
        await sendText(env, serviceUrl, convId, `**Phase 3:** ${icon(results.teams)} Teams Chat`);
      }
      // Phase 4 (Notion link patching) is silent — summary card follows immediately
    } catch (progressErr) {
      console.error(`[provisioner] Failed to send ${phase} progress:`, progressErr.message);
    }
  };
