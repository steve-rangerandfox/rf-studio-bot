 ...
 *
 * Recognised commands:
 *   /storyboard | "new storyboard" | "start"  — show upload prompt card
  ...
  if (
    ...
    await sendText(
      ...
      "**Script-to-Storyboard Bot**\n\n" +
        "Commands:\n" +
        "- **/storyboard** or **new storyboard** \u2014 Start creating a storyboard\n" +
        "- **/storyboard resume {jobId}** \u2014 Resume a failed Boords job\n" +
    ...
    await sendText(
      ...
      serviceUrl,
      convId,
      "Operation cancelled. Say **/storyboard** or **new storyboard** to start again."
    );
  } else {
    await sendText(
      env,
      serviceUrl,
      convId,
      'Send **/storyboard** or **new storyboard** to start creating a storyboard from your script.'
    );
  }
}
  ...
  if (action === "createStoryboard") {
    ...
    await sendText(
      ...
      `Starting storyboard creation for "${cardValue.projectName || "project"}"...\n` +
        ...
        `Figma/FigJam: ${cardValue.createFigmaAssets !== "false" ? "Yes" : "No"}`
    );
...
async function handleFormSubmission(env, convId, serviceUrl, formData, activity) {
  ...
  const intake = {
    ...
    selectedVoiceId: formData.selectedVoiceId || VOICES[0].voiceId,
    aspectRatio: formData.aspectRatio || "16:9",
    secondsPerFrame: parseInt(formData.secondsPerFrame || "5", 10) || 5,
    createFigmaAssets: formData.createFigmaAssets !== "false",
