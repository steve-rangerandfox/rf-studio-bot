 ...
 * after 24 h (86400 s).
 *
 * Sequential Frame Creation:
 * Frames are created one at a time so Boords auto-assigns sequential
 * numbers (1, 2, 3...). Parallel creation caused race conditions leading
 * to non-sequential numbering. KV checkpoints every 5 frames for resume.
 *
 * API Endpoints:
...
/**
 * Get the frames that already exist in a storyboard.
 * Used to detect and clean up default frames Boords auto-creates.
 *
 * @param {string} apiKey - Boords API key
 * @param {string} storyboardId - Storyboard ID
 * @returns {Promise<Array<{id: string}>>}
 */
async function getExistingFrames(apiKey, storyboardId) {
    const result = await boordsRequest("GET", `/storyboards/${storyboardId}`, apiKey);
    const extracted = extractFromResponse(result);
    // Boords may nest frames under various keys
    const frames = extracted.frames || extracted.frame_list || [];
    if (Array.isArray(frames)) {
          return frames.map((f) => {
                const inner = f.data ? f.data : f;
...
/**
 * Delete a single frame from a storyboard.
 *
 * @param {string} apiKey - Boords API key
 * @param {string} frameId - Frame ID to delete
 */
async function deleteFrame(apiKey, frameId) {
    await boordsRequest("DELETE", `/frames/${frameId}`, apiKey);
}

/**
 * Remove any default frames that Boords auto-creates with a new storyboard.
 * This ensures our frames get clean sequential numbering starting from 1.
 *
 * @param {string} apiKey - Boords API key
 * @param {string} storyboardId - Storyboard ID
 */
async function clearDefaultFrames(apiKey, storyboardId) {
    try {
          const existing = await getExistingFrames(apiKey, storyboardId);
          if (existing.length > 0) {
                console.log(`[boords] Clearing ${existing.length} default frame(s) from new storyboard`);
                await Promise.all(
                      existing.map((f) => deleteFrame(apiKey, f.id).catch((err) => {
                            console.warn(`[boords] Failed to delete default frame ${f.id}:`, err.message);
                      }))
                );
          }
    } catch (err) {
          console.warn("[boords] Could not clear default frames (non-fatal):", err.message);
    }
}

/**
 * Create a single frame in a Boords storyboard.
 * Does NOT send a `number` field — Boords auto-assigns sequential numbers.
 * Frames are created in order so auto-numbering matches our intended sequence.
 *
 * @param {string} apiKey - Boords API key
 ...
 * 3. Clear any default frames Boords auto-creates (fixes numbering)
 * 4. Save an initial checkpoint to KV (so resume is possible even before frames start)
 * 5. Create frames sequentially (so Boords auto-numbers them 1, 2, 3...)
 * 6. Delete the checkpoint on success (clean up KV)
 * 7. Return the storyboard edit URL
 *
...
export async function createBoordsStoryboard(
  ...
  // Step 3: Clear any default frames Boords auto-creates with new storyboards.
  // This ensures our frames get clean sequential numbering starting from 1.
  await clearDefaultFrames(apiKey, storyboard.id);

  // Step 4: Save initial checkpoint before we start the frame loop
  ...
  // Step 5: Create frames sequentially so Boords auto-numbers them in order.
  // (Parallel creation caused non-sequential numbering due to race conditions.)
  if (onProgress) {
        await onProgress(
  ...
  for (let idx = 0; idx < scenes.length; idx++) {
    ...
    try {
      ...
      const remainingScenes = scenes.slice(idx);
      await saveCheckpoint(kv, activeJobId, {
            jobId: activeJobId,
            storyboardId: storyboard.id,
            editUrl: storyboard.editUrl,
      ...
      throw new Error(
        `Boords frame creation stopped at frame ${idx + 1}/${scenes.length}: ${errMsg}. ` +
        `Resume with: /storyboard resume ${activeJobId}`
      );
    }

    // Checkpoint every BATCH_SIZE frames
    if ((idx + 1) % BATCH_SIZE === 0 || idx === scenes.length - 1) {
      const remainingAfter = scenes.slice(idx + 1);
      await saveCheckpoint(kv, activeJobId, {
            jobId: activeJobId,
            storyboardId: storyboard.id,
            editUrl: storyboard.editUrl,
    ...
    // Progress update at halfway point to conserve subrequests
    const created = idx + 1;
    if (onProgress && created === Math.ceil(scenes.length / 2) && created < scenes.length) {
      await onProgress(
  ...
  // Step 6: All frames created — delete checkpoint (clean up KV)
  if (kv) {
    try {
...
export async function resumeBoordsFrameCreation(env, jobState, onProgress) {
  ...
  for (let idx = 0; idx < remainingScenes.length; idx++) {
    const scene = remainingScenes[idx];
    const frameNum = (startIndex || 0) + idx + 1;
    ...
    try {
      ...
      if (jobId && kv) {
        const stillRemaining = remainingScenes.slice(idx);
        await saveCheckpoint(kv, jobId, {
          ...jobState,
          remainingScenes: stillRemaining,
          startIndex: (startIndex || 0) + idx,
          failedAt: (startIndex || 0) + idx + 1,
        });
      }
      const errMsg = frameErr instanceof Error ? frameErr.message : String(frameErr);
      throw new Error(
    ...
    // Checkpoint every BATCH_SIZE frames
    if (jobId && kv && ((idx + 1) % BATCH_SIZE === 0 || idx === remainingScenes.length - 1)) {
      const stillRemaining = remainingScenes.slice(idx + 1);
      await saveCheckpoint(kv, jobId, {
        ...jobState,
        remainingScenes: stillRemaining,
        startIndex: (startIndex || 0) + idx + 1,
      });
    }
  }
