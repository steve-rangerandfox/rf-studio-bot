...
export async function createBoordsStoryboard(
  ...
  // Step 3: Create frames in parallel batches (order preserved via frame number)
  if (onProgress) {
    await onProgress(
  ...
  for (let batchStart = 0; batchStart < scenes.length; batchStart += BATCH_SIZE) {
    ...
    const created = Math.min(batchStart + BATCH_SIZE, scenes.length);
    if (onProgress && created < scenes.length) {
      await onProgress(
