...
/** Ensure serviceUrl always ends with exactly one slash. */
...
export async function sendCard(env, serviceUrl, conversationId, card) {
  ...
  const base = ensureTrailingSlash(serviceUrl);
...
export async function updateCard(env, serviceUrl, conversationId, activityId, card) {
  const token = await getAccessToken(env);
  const base = ensureTrailingSlash(serviceUrl);
  const url = `${base}v3/conversations/${conversationId}/activities/${activityId}`;
